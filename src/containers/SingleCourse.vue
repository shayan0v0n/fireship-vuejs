...
export default {
    setup() {
        ...
        onMounted(() => {
            axios.get("https://fireship-6470a-default-rtdb.firebaseio.com/courses.json")
                .then(res => {
                courses.value = res.data.find((item) => {
        ...
        const addtocartHandler = () => {
            ...
            currentCart.push(courses.value)
    ...
    components: { LicenseCheckout, LicensePlan, CardPlaceholder }
}

</script>

<template>
    <div class="text-center single-lesson"  :class="[currentTheme == 'dark' ? 'single-lesson-dark' : null]" v-if="courses.path">
        <div class="row container m-auto py-5">
            <div class="col-12 col-md-9">
                <section class="card" id="product">
                    <img :src="`/assets/imgs/${courses.path}.jpg`" class="w-100" alt="">
                    <div class="card-body text-start">
                        <h2 class="card-title">{{ courses.title }}</h2>
                        <p class="card-text">{{ courses.desc }}</p>
                        <p class="card-text price">PRICE: <span>${{ courses.price }}.00</span></p>
                    </div>
                </section>
